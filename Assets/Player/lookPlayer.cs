...
public class lookPlayer : MonoBehaviour
{
    ...
    private void Start()    {Cursor.lockState = CursorLockMode.Locked; }    // Lock the cursor to the center of the screen
    void Update()
    {  
        float mouseX = lookInput.x * mouseSensitivity * Time.deltaTime;     // Get the mouse input and apply sensitivity
        float mouseY = lookInput.y * mouseSensitivity * Time.deltaTime;     // Get the mouse input and apply sensitivity
    ...
    public void OnLook(InputAction.CallbackContext context)
        {lookInput = context.ReadValue<Vector2>(); }                        // Get the input value from the context and store in lookInput
}
